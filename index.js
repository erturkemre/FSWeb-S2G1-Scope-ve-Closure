...
/*Görev 1: macSkoru()
  ...
  skor1 skorArtirici fonksiyonunun return fonksiyonunu döner, closure fonksiyondur. skor2 direkt işlem yapan fonksiyondur
  
  2. Hangisi bir closure kullanmaktadır? Nasıl tarif edebilirsin? (yarınki derste öğreneceksin :) )
  skor1 closure dur returnünde fonksiyon döner.
  
  3. Hangi durumda skor1 tercih edilebilir? Hangi durumda skor2 daha mantıklıdır?
  içerdeki fonksiyon parent fonksiyondan gelecek parametreyi kullanacaksa skor1 tercih edilmeli.Direkt işlem yapılıp döndürülcekse skor2 daha mantıklı
*/
...
function skorTabelasi(periyotSkoru, takimSkoru, ceyrekSayisi) {
  ...
  for (let i = 0; i < ceyrekSayisi; i++) {
    ...
    macSonucu.push(
      `${i + 1}. Periyot: Ev Sahibi ${periyot.EvSahibi} - Konuk Takım ${
        periyot.KonukTakim
      }`
    );
  ...
  while (EvSahibi === KonukTakim) {
    let uzatmaSayisi = 1;
    let periyot = periyotSkoru(takimSkoru);
    let uzatma = `Uzatma ${uzatmaSayisi}: Ev Sahibi ${periyot.EvSahibi} - Konuk Takım ${periyot.KonukTakim}`;
